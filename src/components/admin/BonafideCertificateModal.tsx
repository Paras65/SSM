  ...
  return (
    <div className="printable-modal fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4 bg-stone-900/70 backdrop-blur-xs overflow-hidden print:static print:p-0 print:m-0 print:bg-transparent print:overflow-visible">
      <div className="bg-white rounded-2xl sm:rounded-3xl max-w-3xl w-full shadow-2xl border border-stone-200 overflow-hidden flex flex-col max-h-[92vh] sm:max-h-[90vh] print:max-h-none print:shadow-none print:border-none print:max-w-none print:rounded-none print:m-0">
        ...
        {/* Printable Certificate - Scrollable */}
        <div className="flex-1 overflow-y-auto p-3 sm:p-6 print:p-0 print:overflow-visible">
          ...
          <div className="py-4 sm:py-8 space-y-4 sm:space-y-6 text-xs sm:text-sm text-stone-800 leading-relaxed sm:leading-loose">
            <p className="text-justify indent-4 sm:indent-8">
              प्रमाणित किया जाता है कि <strong>{student.gender === 'Bhaiya' ? 'भैया' : 'बहिन'} {student.name}</strong>, सुपुत्र/सुपुत्री श्री <strong>{student.fatherName}</strong> एवं श्रीमती <strong>{student.motherName || student.fatherName}</strong>, वर्तमान शैक्षणिक सत्र <strong>{currentSchool.currentAcademicYear || '2025-26'}</strong> में हमारे विद्यालय की कक्षा <strong>{student.class}</strong>, वर्ग <strong>'{student.section || 'A'}'</strong> के नियमित एवं प्रामाणिक विद्यार्थी (Bonafide Student) हैं।
            </p>

            <div className="bg-stone-50 p-3 sm:p-4 rounded-xl border border-stone-200 grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3 text-xs">
              <div className="flex sm:block justify-between items-center">
                <span className="text-stone-500 font-bold uppercase block text-[10px]">अनुक्रमांक (Roll No.)</span>
                <span className="font-bold text-stone-900 text-xs sm:text-sm font-mono">{student.rollNo}</span>
              </div>
              <div className="flex sm:block justify-between items-center">
                <span className="text-stone-500 font-bold uppercase block text-[10px]">प्रवेश तिथि (Admission Date)</span>
                <span className="font-bold text-stone-900 text-xs sm:text-sm">{student.admissionDate || '01/07/2023'}</span>
              </div>
              <div className="flex sm:block justify-between items-center">
                <span className="text-stone-500 font-bold uppercase block text-[10px]">जन्म तिथि (DOB)</span>
                <span className="font-bold text-stone-900 text-xs sm:text-sm">{student.dob || '15/08/2012'}</span>
              </div>
              <div className="flex sm:block justify-between items-center">
                <span className="text-stone-500 font-bold uppercase block text-[10px]">रक्त समूह (Blood Group)</span>
                <span className="font-bold text-red-700 text-xs sm:text-sm">{student.bloodGroup || 'B+'}</span>
              </div>
            </div>
