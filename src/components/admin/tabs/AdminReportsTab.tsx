...
const AdminReportsTabComponent: React.FC<AdminReportsTabProps> = ({
  ...
  const [reportToDelete, setReportToDelete] = useState<ReportCard | null>(null);
  ...
  return (
    <div className="bg-white rounded-2xl border border-stone-200 p-6 space-y-6">
      {/* Optional Pro Banner for Free Plan */}
      {!isPro && (
        <div className="bg-gradient-to-r from-amber-50 to-orange-50 border border-amber-200 rounded-2xl p-3.5 px-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shadow-2xs">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <p className="text-xs font-bold text-stone-900">
                विद्या भारती प्रो फीचर: 360° समग्र प्रगति पत्र एवं NEP 2020 मूल्यांकन
              </p>
              <p className="text-[11px] text-stone-500">
                पंचमुखी संस्कार (योग, शारीरिक, संगीत, संस्कृत, नैतिक शिक्षा) युक्त विस्तृत मार्कशीट व AI टिप्पणियां अनलॉक करें।
              </p>
            </div>
          </div>
          <button
            ...
            className="px-3 py-1.5 bg-amber-600 hover:bg-amber-700 text-white font-bold text-xs rounded-xl shadow-xs inline-flex items-center gap-1.5 transition shrink-0 cursor-pointer self-start sm:self-auto"
          >
            <Crown className="w-3.5 h-3.5 text-yellow-200 fill-yellow-300" />
            <span>प्रो अनलॉक करें</span>
          </button>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/50 backdrop-blur-xs">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-stone-200 space-y-4">
            <div className="flex items-center gap-3 text-rose-600">
              ...
              <div>
                <h4 className="font-bold text-stone-900">प्रगति पत्र हटाएं</h4>
                <p className="text-xs text-stone-500">स्थायी विलोपन पुष्टि</p>
              </div>
            </div>
            <p className="text-xs text-stone-700 leading-relaxed">
              क्या आप निश्चित रूप से <strong>"{studentMap.get(reportToDelete.studentId)?.name || 'छात्र'}"</strong> का <strong>{reportToDelete.examTerm}</strong> (सत्र {reportToDelete.academicYear}) का प्रगति पत्र स्थायी रूप से हटाना चाहते हैं?
            </p>
            <div className="flex justify-end gap-2 pt-2">
              ...
              <button
                type="button"
                onClick={() => {
                  deleteReportCard(reportToDelete.id);
