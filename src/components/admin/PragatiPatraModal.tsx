...
export const PragatiPatraModal: React.FC<PragatiPatraModalProps> = ({ reportCard, student, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 backdrop-blur-xs p-2 sm:p-4 overflow-y-auto print:static print:p-0 print:m-0 print:bg-transparent print:overflow-visible">
      <div className="bg-white rounded-3xl shadow-2xl max-w-4xl w-full overflow-hidden border border-orange-300 print:shadow-none print:border-none print:max-w-none print:rounded-none print:p-0 print:m-0 my-4 print:my-0">
        
        {/* Action Header (Hidden in Print) */}
        <div className="no-print bg-gradient-to-r from-orange-800 via-amber-800 to-orange-900 text-white px-4 sm:px-6 py-3 flex flex-wrap items-center justify-between gap-2.5">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 text-sm font-bold tracking-tight">
              <Award className="w-4 h-4 text-yellow-300" />
              <span>प्रगति पत्र • NEP 2020 HPC</span>
            </div>

            {/* Layout Switcher Tabs */}
            <div className="inline-flex rounded-lg bg-black/25 p-0.5 text-xs font-semibold">
              <button
                type="button"
          ...
          <div className="flex items-center space-x-2">
            {activeLayout === 'hpc' && (
              <>
                ...
                >
                  ...
                  <span>{isEditing ? 'पूर्वावलोकन (Preview)' : 'संपादित करें (Edit)'}</span>
                </button>
                ...
                {isEditing && (
                  <button
                    ...
                    disabled={isSaving}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer disabled:opacity-50"
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 bg-emerald-700 hover:bg-emerald-600 text-white rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer"
            ...
            <button
              type="button"
              onClick={handlePrint}
              className="flex items-center gap-1.5 px-3 py-1.5 bg-white text-orange-900 hover:bg-orange-50 rounded-lg text-xs font-bold transition-all shadow-xs cursor-pointer"
            >
            ...
            <button
              type="button"
              onClick={onClose}
              className="p-1.5 rounded-lg hover:bg-orange-950 text-white transition-all cursor-pointer"
              aria-label="Close"
            >
              <X className="w-5 h-5" />
            </button>
